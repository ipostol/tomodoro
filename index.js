...
async function loadStatistics(updateEntryCards = true) {
  ...
  statSummaryAverage.innerText = hmstrFull(
    totalValue / (rec.length === 0 ? 1 : rec.length)
  );
  statSummaryShortest.innerText = hmstrFull(minRoundValue);
  statSummaryLongest.innerText = hmstrFull(maxRoundValue);

  if (totalValue === 0) {
    remarkDaily.style.display =
      remarkHourly.style.display =
      remarkMonthly.style.display =
        "none";
  } else {
    remarkDaily.style.display =
      remarkHourly.style.display =
    ...
    remarkHourly.querySelector(".remark-value").innerText =
      hourlyFullNames[maxH];
  ...
  charts
    .sort((a, b) => b.t - a.t)
    .forEach((chart, i) => {
      taskBars[chart.task].querySelector(".stat-value").innerText = hmstr(
        chart.t
      );
      taskBars[chart.task].querySelector(".bar").style.width =
        (chart.t / maxvalue) * 100 + "%";
      ...
      pieCards[chart.task].querySelector(".pie-card-time-text").innerText =
        hmstrFull(chart.t);
      pieCards[chart.task].querySelector(".pie-card-rounds-span").innerText =
        chart.n;
      ...
      if (totalValue === chart.t) {
        pie.setAttribute(
          "d",
          `M60,10 A50 50 0 1 1 60 110 A50 50 0 1 1 60 10 z`
        );
        pie.dataset.circle = true;
      } else {
        pie.setAttribute(
          "d",
          `M60,60 L${p1.join(",")} A50 50 0 ${
            b - a > Math.PI ? 1 : 0
          } 1 ${p2.join(" ")} L60,60 z`
        );
      }
...
function pathPointerEnter(ev) {
  ...
  if (!ev.target.dataset.circle) {
    ...
    ev.target.style.transform = `translate(${Math.sin(a) * 5}px, ${
      -Math.cos(a) * 5
    }px)`;
  }
...
function hmstrFull(t) {
  ...
  return `${r[0]} Hour${r[0] > 1 ? "s" : ""} & ${r[1]} Minute${
    r[1] > 1 ? "s" : ""
  }`;
}
